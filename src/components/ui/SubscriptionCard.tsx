...
import { useAuth, usePlanSelection, usePayment, billingPeriods } from '@/hooks/useSubscription';
import LoginModal from '@/Login/login-modal';
import { message } from '@/components/ui/CustomMessage';
import type { SubscriptionCardProps, Plan, UserInfo, PaymentData, ApiError } from '@/types/subscription';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!plan || !plan.packageFeatureId) {
        message.error('Plan information is missing.');
        setProcessing(false);
        return;
      }
      ...
      if (!packageId) {
        message.error('Package ID not found for selected period.');
        setProcessing(false);
        return;
      }
...
const SubscriptionCard: React.FC<SubscriptionCardProps> = ({ 
  ...
  const plans = propPlans || planSelection.plans;
  const selectedPeriod = propSelectedPeriod || planSelection.selectedPeriod;
  ...
  const handlePeriodChange = (periodId: 'yearly' | 'monthly') => {
    if (!propSelectedPeriod) {
      planSelection.handlePeriodChange(periodId);
    }
  };

  // 处理支付提交
  const handlePaymentSubmit = async (paymentData: PaymentData) => {
    await payment.handlePaymentSubmit(paymentData);
  };
  ...
  return (
    <>
                ...
                >
                  {plan.popular && (
                    <div className="absolute -top-5 left-1/2 -translate-x-1/2">
                      {/* 方案1: 紫色到红色渐变 (原版) */}
                      <div className={`bg-gradient-to-r from-purple-500 to-rose-500 text-white px-4 sm:px-5 py-2 rounded-full text-sm font-bold shadow-lg ${currentTheme === 'dark' ? 'shadow-purple-500/20' : 'shadow-purple-400/25'}`}>
                        MOST POPULAR ✨
                      </div>
      ...
      <Elements stripe={stripePromise}>
        <PaymentModal
          visible={payment.showPaymentModal}
